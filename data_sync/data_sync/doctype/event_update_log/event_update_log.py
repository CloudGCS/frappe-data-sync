...
class EventUpdateLog(Document):
	def after_insert(self):
		...
		enqueued_method = (
			"data_sync.data_sync.doctype.event_consumer.event_consumer.notify_event_consumers"
		)
		jobs = get_jobs()
...
ENABLED_DOCTYPES_CACHE_KEY = "data_sync_enabled_doctypes"

def check_doctype_has_consumers(doctype: str) -> bool:
	"""Check if doctype has event consumers for data sync"""
	def fetch_from_db():
...
def get_update_logs_for_consumer(event_consumer, doctypes, last_update):
	...
	from data_sync.data_sync.doctype.event_consumer.event_consumer import has_consumer_access
