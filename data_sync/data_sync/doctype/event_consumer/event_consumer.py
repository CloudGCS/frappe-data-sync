...
class EventConsumer(Document):
	...
	def clear_cache(self):
		from data_sync.data_sync.doctype.event_update_log.event_update_log import (
...
def notify(consumer):
	...
	if consumer_status == "online":
		try:
			...
			client.post_request(
				{
					"cmd": "data_sync.data_sync.doctype.event_producer.event_producer.new_event_notification",
	...
	if not consumer.flags.notified:
		enqueued_method = "data_sync.data_sync.doctype.event_consumer.event_consumer.notify"
		jobs = get_jobs()
